    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if(StringUtils.hasText(token) && this.jwtTokenService.validateToken(token)) {
                final String userId = this.jwtTokenService.getUserIdFromToken(token);
                final String tenantId = this.jwtTokenService.getTenantIdFromToken(token);
                final   String role = this.jwtTokenService.getRoleFromToken(token);
                ...
                log.debug("User authenticated for user Id: {}, and tenantId: {}, role: {}", userId, tenantId, role);
            }
